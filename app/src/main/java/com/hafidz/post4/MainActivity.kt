...
class MainActivity : AppCompatActivity() {
    ...
    private fun simpanData() {
        ...
        if (nama.isEmpty() || nik.isEmpty() || kab.isEmpty() || kec.isEmpty() ||
            ...
            Snackbar.make(btnSimpan, "Semua data nya diisi dulu mas!!", Snackbar.LENGTH_SHORT).show()
            return
        }

        val isInserted = db.insertWarga(nama, nik, kab, kec, desa, rt, rw, gender, status)
        if (isInserted) {
            Snackbar.make(btnSimpan, "Data berhasil disimpan!", Snackbar.LENGTH_SHORT).show()
            resetInput()
            tampilkanData()
        } else {
            Snackbar.make(btnSimpan, "Gagal menyimpan data!", Snackbar.LENGTH_SHORT).show()
        }
    }
    ...
    private fun hapusSemuaData() {
        db.deleteAllWarga()
        tampilkanData()
        Snackbar.make(btnReset, "Semua data berhasil dihapus!", Snackbar.LENGTH_SHORT).show()
    }
}